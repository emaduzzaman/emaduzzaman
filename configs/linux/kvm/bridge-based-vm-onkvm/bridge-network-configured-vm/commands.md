```
  ...
  399  bridge link
  400  bridge fdb show br br0
  401  openstack port list --server gelani-lab-2
  402  sudo virsh destroy br-vm1
  403  clear
  404  sudo virsh undefine br-vm1
  405  sudo virt-install   --name br-vm1   --memory 2048   --vcpus 2   --disk path=/var/lib/libvirt/images/vms/br-vm1/br-vm1.qcow2,format=qcow2   --disk path=/var/lib/libvirt/images/vms/br-vm1/seed.iso,device=cdrom   --os-variant ubuntu24.04   --network type=direct,source=ens3,source_mode=bridge,model=virtio   --graphics none   --console pty,target_type=serial   --import
  406  clear
  407  sudo virsh destroy br-vm1 2>/dev/null || true
  408  sudo virsh undefine br-vm1 2>/dev/null || true
  409  sudo rm -rf /var/lib/libvirt/images/vms/br-vm1
  410  cat | sudo tee /tmp/br0-net.xml >/dev/null <<'EOF'
<network>
  <name>br0-net</name>
  <forward mode='bridge'/>
...
EOF
  ...
  411  sudo virsh net-define /tmp/br0-net.xml
  412  sudo virsh net-start br0-net
  413  sudo virsh net-autostart br0-net
  414  sudo virsh net-list --all
  415  sudo mkdir -p /var/lib/libvirt/images/vms/br-vm1
  416  sudo chown -R libvirt-qemu:kvm /var/lib/libvirt/images/vms/br-vm1
  417  sudo chmod 750 /var/lib/libvirt/images/vms/br-vm1
  418  cd /var/lib/libvirt/images/vms/br-vm1
  419  sudo qemu-img create -f qcow2   -F qcow2 -b /var/lib/libvirt/images/_base/ubuntu-24.04-noble-base.qcow2   br-vm1.qcow2 20G
  420  ping -c 2 192.168.95.191 || true
  421  ip neigh | grep 192.168.95.191 || true
  422  cat | sudo tee user-data >/dev/null <<'EOF'
#cloud-config
ssh_pwauth: true
chpasswd:
  list: |
...
EOF
  ...
  423  cat | sudo tee meta-data >/dev/null <<'EOF'
instance-id: br-vm1
local-hostname: br-vm1
EOF

  424  cat | sudo tee network-config >/dev/null <<'EOF'
version: 2
ethernets:
  enp1s0:
    dhcp4: false
...
EOF
  ...
  425  sudo cloud-localds br-vm1-seed.iso user-data meta-data --network-config=network-config
  426  sudo chown libvirt-qemu:kvm br-vm1-seed.iso user-data meta-data network-config
  427  sudo chmod 640 br-vm1-seed.iso user-data meta-data network-config
  428  sudo virt-install   --name br-vm1   --memory 2048   --vcpus 2   --disk path=$PWD/br-vm1.qcow2,format=qcow2,bus=virtio   --disk path=$PWD/br-vm1-seed.iso,device=cdrom   --os-variant ubuntu24.04   --network network=br0-net,model=virtio   --graphics none   --console pty,target_type=serial   --import   --noautoconsole
  429  sudo virsh list --all
  430  ping -c 3 192.168.95.191
  431  ssh ubuntu@192.168.95.191
  432  history
``` 

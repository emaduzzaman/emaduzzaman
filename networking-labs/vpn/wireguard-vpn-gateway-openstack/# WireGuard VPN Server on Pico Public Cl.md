# WireGuard VPN Gateway Implementation 
## Production-Grade Secure Remote Access Infrastructure on Pico Public Cloud (OpenStack)
...
This document details the implementation of a **Layer 3 WireGuard VPN gateway** that establishes an encrypted IPsec-alternative tunnel, enabling cryptographically authenticated remote endpoints to access RFC1918 private cloud workloads (192.168.90.0/27) on Pico Public Cloud via a centralized NAT gateway with a single public IP (160.191.150.171:51820/UDP).
...
#### 1.4 Launch Instance
```
Navigation: Compute → Instances → Virtual Instance
...
```
Navigation: compute → Virtual Instances → click the virtual instance (wireguard) → Floating IPs → Attatch IP

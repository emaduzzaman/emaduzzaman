# WireGuard VPN Gateway on OpenStack Cloud
...
### Prerequisites
- Ubuntu Server 24.04 LTS VM on OpenStack cloud
...
```bash
# 1. Clone repository
git clone https://github.com/yourusername/wireguard-vpn-gateway.git
...
# 4. Copy configuration (edit with your keys and IPs)
sudo cp configuration-files/server/wg0.conf /etc/wireguard/
...
sudo bash configuration-files/firewall/iptables-rules-wireguard.sh
